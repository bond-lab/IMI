...
# Wilson(FIXME): Undefined variable valid_username
...
def tagbox(sss, cid, wp, tag, ntag, com):
    ...
    tagbox_text.append("<input type='hidden' name='oldtag_%s' value='%s' />\n" % (cid, tag if tag else ''))
    tagbox_text.append("<input type='hidden' name='oldcom_%s' value='%s' />\n" % (cid, com if com else ''))
...
### process the tags
jilog("Start processing all the tags --------------- [TA1412_01]")
jilog("im tagging cdis=%s" % (cids,))
#c.execute("begin") # 2015-01-04 [Tuan Anh] We might need to use transaction, I'll look at this later
for cd in cids.split(' '): ### prefer local ntag
    ...
    if com:
        ...
        com = ''
    # 2014-12-30 [Tuan Anh]
    # Compare com & oldcom, tag & oldtag to prevent unnecessary changes
    oldcom = form.getfirst("oldcom_%d_%d" % (sid, cid), None)
    if oldcom:
        oldcom = oldcom.strip()
    else:
        oldcom = ''
    oldtag = form.getfirst("oldtag_%d_%d" % (sid, cid), None)
    ...
    if tag: ### then local tag
        if tag != oldtag:
            ...
            #jilog('im NOT tagging tag=%s - sid=%s - cid=%s because it is NOT changed [TA1412_01]' % (tag, sid, cid))
            pass

    # LMC: we're getting rid of dtag all together!
    ...
    if com != oldcom:
        ...
        pass
#END for (process the tags)
jilog("End processing all the tags --------------- [TA1412_01]")
#c.execute('commit')
...
template_data.update({
    ...
    'lang': lang,
    'lim': lim,
    'usrname_logout': usrname,
    'wn_src': '%s?usrname=%s&gridmode=%s&lang=%s&lemma=%s&ss=%s' % \
        (wncgi, usrname, gridmode, lang,lemma,tsss)
})
...
if totag:
    ...
    cids = u" ".join(["%s|%s" % (r[0], r[1]) for r in totag])  ### all the cids
    ...
    for a_tag in tag_freqs_order:
        if a_tag in a_tag in item_text_by_tag and tag_freqs:
    ...
    if len(all_comments) > 0:
        ...
        all_comments_text = re.sub(r'([0-9]{8}-[varn])', r"""<a href='wn-gridx.cgi?synset=\1&gridmode=%s' target='wn'>\1</a> [wn_list_loc(\1)]""" % gridmode, all_comments_text)
            ...
            for lcid in show[ltag][lsid]:
                ...
                for wid, word, pos, lemma in sentences_words[int(lsid)]:
                    if wid in cwds:
                        ...
                        # 2014-12-08 FCB If you click a word, look at the sentence
                        tt = '''\
%s?lemma=%s&corpus=%s&lang=%s&usrname=%s&gridmode=%s&sid=%d&wid=%d''' % \
                            ("tag-word.cgi", lemma,  corpus, lang, 
                             usrname, gridmode,
                             lsid, wid)
...
################################################################################
# CASE 2: THE LEMMA WAS NOT FOUND (AND, OF COURSE, NO CONCEPTS HAVE IT)
################################################################################
elif not lemma:  # show search form
    HTML.render_template('tag-lexs-search.htm', template_data, cookie_text)

################################################################################
# CASE 3: THE LEMMA WAS FOUND BUT IT DOES NOT BELONG TO A CONCEPT
################################################################################
else:
    ### 
